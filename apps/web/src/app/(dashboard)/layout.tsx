...
import { Share2, Megaphone, Users, Settings, Menu, X, Bell, LogOut, ChevronRight, Webhook, CreditCard } from 'lucide-react'
    ...
    <div className="flex flex-col h-full">
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} onClick={onClose}
              className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors group ${active ? 'text-violet-300' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200'}`}
              ...
              {label}
              {active && <ChevronRight className="ml-auto w-3 h-3 text-violet-400 opacity-60" />}
            </Link>
          )
        })}
      </nav>
      <div className="border-t border-white/10 px-3 py-3">
        <div className="flex items-center gap-3 px-2 py-2">
          <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold text-white" style={{ background: `linear-gradient(135deg, ${BRAND}, #5b21b6)` }}>U</div>
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        <header className="flex items-center h-14 px-4 border-b border-white/[0.07] bg-gray-950 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400">
            <span className="text-gray-600">ReferKit</span>
